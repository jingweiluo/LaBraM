...
import torch
import torch.distributed as dist # 分布式训练框架
...
def build_pretraining_dataset(datasets: list, time_window: list, stride_size=200, start_percentage=0, end_percentage=1):
    # start_percentage是一个百分比，定义了样本从哪个位置开始截取。具体来说，假设能划出10个trial，start_percentage = 0.2的话，那就舍弃前两个，从第三个Trial开始截取
    # 同理，假设end_percentage=0.6，那就到第6个Trial,之后就不要了
    # ( int（total_num_trials * start_percentage), int（total_num_trials * end_percentage) ]
    # (2, 6]
    shock_dataset_list = [] # 包含不同规范的（32导, 64导的dataset）
    ch_names_list = [] # 同上
    for dataset_list, window_size in zip(datasets, time_window):
        ...
        # param1 =  [Path("path/to/dataset1"), Path("path/to/dataset2")], param2 = 4 * 200, param3 = 200
